...
;; STRUCTURE C : AlGaN/GaN MIS-HEMT, Al2O3/HfO2 STACK
;;               Al2O3 = 3 nm (interfacial, on AlGaN)
;;               HfO2  = 7 nm (on top of Al2O3)
;;               Total dielectric = 10 nm
;;
...
;;   -X up toward air. Source=LEFT, Drain=RIGHT, Substrate=BOTTOM.
;;   NOT rotated: horizontal slabs stacked vertically.
;;==================================================================;;
...
(define t_al2o3  0.003)   ; Al2O3 interfacial layer (3 nm) -- STRUCTURE C
(define t_hfo2   0.007)   ; HfO2 high-k (7 nm)          -- STRUCTURE C
...
(define x_surf      0.0)
(define x_barr_bot  (+ x_surf     t_barr))
(define x_chan_bot  (+ x_barr_bot t_chan))
(define x_bbarr_bot (+ x_chan_bot t_bbarr))
(define x_cbuf_bot  (+ x_bbarr_bot t_cbuf))
(define x_buf_bot   (+ x_cbuf_bot t_buf))
(define x_aln_bot   (+ x_buf_bot  t_aln))
(define x_sic_bot   (+ x_aln_bot  t_sic))

;; dielectric stack (negative X, toward air)
(define x_al2o3_top (- x_surf      t_al2o3))   ; -0.003
(define x_hfo2_top  (- x_al2o3_top t_hfo2))    ; -0.010
(define x_gate_top  (- x_hfo2_top  t_gm))      ; -0.060
(define x_sdm_top   (- x_surf      t_sdm))     ; -0.050

;;-------- 3. LATERAL (Y) COORDINATES (source left -> drain right) -;;
(define ys1 0.0)
(define ys2 L_src)
(define yg1 (+ L_src L_sg))
(define yg2 (+ yg1 L_gate))
(define yd1 (+ yg2 L_gd))
(define yd2 (+ yd1 L_drn))
(define Ymax yd2)
(define ymid (/ Ymax 2.0))
...
;;-------- 5. GATE DIELECTRIC STACK (Al2O3 then HfO2) --------------;;
;; Al2O3 interfacial layer (sits directly on AlGaN)
(sdegeo:create-rectangle (position x_al2o3_top ys2 0) (position x_surf yd1 0)
                         "Aluminum2O3" "R_al2o3")
;; HfO2 high-k (on top of Al2O3)
(sdegeo:create-rectangle (position x_hfo2_top ys2 0) (position x_al2o3_top yd1 0)
                         "HfO2" "R_hfo2")
...
;; Gate metal sits on top of HfO2
(sdegeo:create-rectangle (position x_gate_top yg1 0) (position x_hfo2_top yg2 0)
...
;; (1) AlGaN barrier / GaN channel interface (2DEG)
...
;;-------- 11. BUILD MESH & SAVE ----------------------------------;;
(sde:build-mesh "snmesh" "" "Structure_C_msh")
(sde:save-model "Structure_C")
