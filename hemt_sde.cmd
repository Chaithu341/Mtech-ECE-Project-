...
(sdegeo:create-rectangle (position x0 y_top 0)    (position Wdev y_SiN_b 0)  "Si3N4" "R.SiN")

;;------------------------------------------------------------------------------
;; 3. CONTACTS
;;    Source & Drain : ohmic, contacting the cap/barrier on top surface.
;;    Gate           : Schottky metal on the SiN (as in Fig.1, T-gate on SiN).
;;    We define contacts on edges of the top surface rather than as metal bodies,
;;    which is the most robust approach in SDE for HEMTs.
;;------------------------------------------------------------------------------
;; Recess source & drain down to the barrier so ohmic contacts hit the 2DEG
;; (self-aligned ohmics, consistent with Shinohara DH-HEMT). Here we keep them
;; on the top surface for simplicity; the 2DEG is contacted via the regions.
...
;; Source contact : top edge from x0 to x_src_r
(sdegeo:set-current-contact-set "source")
(sdegeo:define-2d-contact
  (find-edge-id (position (* 0.5 x_src_r) y_top 0)) "source")

;; Drain contact : top edge from x_drn_l to x_drn_r
(sdegeo:set-current-contact-set "drain")
(sdegeo:define-2d-contact
  (find-edge-id (position (* 0.5 (+ x_drn_l x_drn_r)) y_top 0)) "drain")

;; Gate contact : top edge from x_gate_l to x_gate_r
(sdegeo:set-current-contact-set "gate")
(sdegeo:define-2d-contact
  (find-edge-id (position (* 0.5 (+ x_gate_l x_gate_r)) y_top 0)) "gate")

;;------------------------------------------------------------------------------
;; 4. DOPING  (light n-type UID background; traps applied in SDevice)
;;------------------------------------------------------------------------------
